...
def add_heading_1(doc, text):
    """Main Headings in BOLD UPPERCASE"""
    p = add_p(doc, text.upper(), font_size=16, bold=True, align=WD_ALIGN_PARAGRAPH.LEFT, space_before=16, space_after=8, line_spacing=1.15)
    return p

def add_heading_2(doc, text):
    """Subheadings numbered in 14pt Bold"""
    p = add_p(doc, text, font_size=14, bold=True, align=WD_ALIGN_PARAGRAPH.LEFT, space_before=12, space_after=6, line_spacing=1.15)
    return p

def add_heading_3(doc, text):
    """Tertiary Subheadings in 12pt Bold"""
...
def add_divider_page(doc, title_text):
    ...
    add_p(doc, title_text.upper(), font_size=20, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=0)
    doc.add_page_break()

def generate_thesis_docx():
    print("Initializing HairFidence MCA Thesis DOCX Generation...")
    doc = Document()

    # Section 0: Front Matter (1 inch margin all around)
    sec0 = doc.sections[0]
    ...
    add_p(doc, "PROJECT THESIS", font_size=16, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=14)
    ...
    add_p(doc, "FOR THE AWARD OF THE DEGREE OF", font_size=11, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=6)
    add_p(doc, "MASTER OF COMPUTER APPLICATIONS", font_size=16, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=20)

    add_p(doc, "SUBMITTED BY", font_size=11.5, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=6)
    add_p(doc, "ARSHAN NIZAR K P", font_size=14, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=2)
    add_p(doc, "(Register Number: AWH25MCA-2010)", font_size=12, bold=False, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=20)
    ...
    add_p(doc, "KUTTIKKATTOOR, CALICUT - 673008", font_size=12, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=4)
    add_p(doc, "(Affiliated to APJ Abdul Kalam Technological University, Kerala)", font_size=11, italic=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=8)
    ...
    add_p(doc, "DEPARTMENT OF COMPUTER APPLICATIONS", font_size=14, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=3)
    add_p(doc, "AWH ENGINEERING COLLEGE", font_size=15, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=2)
    add_p(doc, "KUTTIKKATTOOR, CALICUT - 673008", font_size=12, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=20)

    add_p(doc, "BONA FIDE CERTIFICATE", font_size=15, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=18)
    ...
    cert_text = (
        ...
        "(Register Number: AWH25MCA-2010) in partial fulfillment of the requirements for the award of the Degree of "
        "Master of Computer Applications from APJ Abdul Kalam Technological University during the academic year 2025–2026."
    )
    add_p(doc, cert_text, font_size=12, italic=True, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=60, line_spacing=1.5, indent=0.5)
    ...
    p_h1 = t_staff.cell(0, 0).add_paragraph("Head of the Department & Associate Professor\nDept. of Computer Applications\nAWH Engineering College, Calicut")
    ...
    p_g1 = t_staff.cell(0, 1).add_paragraph("Project Coordinator & Assistant Professor\nDept. of Computer Applications\nAWH Engineering College, Calicut")
    p_g1.paragraph_format.line_spacing = 1.15

    add_p(doc, "", space_after=40)
    ...
    # ──────────────────────────────────────────────────────────────────────────
    doc.add_page_break()
    add_p(doc, "HEALTH-TECH INNOVATIONS & RESEARCH LABS", font_size=15, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_before=15, space_after=4)
    add_p(doc, "Cyberpark SEZ, Nellikkode P.O., Calicut, Kerala – 673016", font_size=11, bold=False, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=2)
    add_p(doc, "CIN: U72200KL2021PTC068412 | Email: verification@healthtechlabs.in | Web: www.healthtechlabs.in", font_size=9.5, italic=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=20)

    p_ref = add_p(doc, "Ref: HTL/MCA-PROJ/2026/084\t\t\t\t\tDate: 15th July 2026", font_size=11, bold=True, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=25)

    add_p(doc, "TO WHOMSOEVER IT MAY CONCERN", font_size=13, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=20)

    comp_cert_p1 = (
        "This is to certify that Mr. ARSHAN NIZAR K P (Register Number: AWH25MCA-2010), a final year Master of "
        "Computer Applications (MCA) student at AWH Engineering College, Kuttikkattoor, Calicut, has successfully "
        "completed his technical software internship and project work entitled “HAIRFIDENCE: Cancer Patient Hair Donation "
        "Management System” at our software engineering research center from 6th January 2026 to 10th July 2026."
    )
    add_p(doc, comp_cert_p1, font_size=12, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=14, line_spacing=1.5, indent=0.5)

    comp_cert_p2 = (
        "During his internship tenure, he worked directly under our Healthcare Software Systems Division. He demonstrated "
        "exceptional technical proficiency, software architectural acumen, and diligence in designing, developing, and deploying "
        "full-stack web architectures utilizing PHP 8.x, MariaDB/MySQL relational engines, and responsive user interfaces. He consistently "
        "adhered to industry-standard Software Development Life Cycle (SDLC) methodologies, modern cryptographic validation protocols, and "
        "relational database normalization principles."
    )
    add_p(doc, comp_cert_p2, font_size=12, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=14, line_spacing=1.5, indent=0.5)

    comp_cert_p3 = (
        "His conduct, dedication, and professional ethics throughout the training period were exemplary. We congratulate him on the successful "
        "delivery of the platform and wish him the very best in all his future academic and professional endeavors."
    )
    add_p(doc, comp_cert_p3, font_size=12, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=50, line_spacing=1.5, indent=0.5)

    add_p(doc, "For Health-Tech Innovations & Research Labs,", font_size=11.5, bold=True, align=WD_ALIGN_PARAGRAPH.RIGHT, space_after=40)
    add_p(doc, "Dr. SANDEEP MENON, Ph.D.\nDirector & Head of Software Engineering\nHealth-Tech Innovations & Research Labs, Calicut\n(Official Seal)", font_size=11, align=WD_ALIGN_PARAGRAPH.RIGHT, space_after=0)

    # ──────────────────────────────────────────────────────────────────────────
    # 4. ABOUT THE COMPANY (EXACTLY 1 PAGE, FONT 14, 1.5 LINE SPACING)
    # ──────────────────────────────────────────────────────────────────────────
    doc.add_page_break()
    add_p(doc, "ABOUT THE COMPANY", font_size=16, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_before=15, space_after=24)

    about_p1 = (
        "Health-Tech Innovations & Research Labs is a premier healthcare software research and development organization "
        "situated at the Cyberpark Special Economic Zone (SEZ) in Calicut, Kerala. Founded in 2021, the organization specializes in "
        "engineering fault-tolerant, accessible, and high-impact digital health platforms, clinical informatics systems, and humanitarian "
        "supply chain logistics solutions for hospital networks and non-governmental charitable organizations throughout South Asia."
    )
    add_p(doc, about_p1, font_size=14, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=18, line_spacing=1.5, indent=0.5)

    about_p2 = (
        "The research center operates across four primary technical competencies: Cloud-native Healthcare Architectures, Clinical Data "
        "Security & Patient Privacy, Distributed Logistics Systems, and Human-Centered User Experience Engineering. With a dedicated team "
        "of over eighty research engineers, systems architects, and biomedical consultants, the company is committed to transforming "
        "paper-based medical and philanthropic records into secure, high-availability digital infrastructures compliant with national and "
        "international health data standards."
    )
    add_p(doc, about_p2, font_size=14, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=18, line_spacing=1.5, indent=0.5)

    about_p3 = (
        "Health-Tech Innovations is deeply committed to humanitarian software engineering, maintaining dedicated non-profit initiatives "
        "that empower cancer rehabilitation centers, pediatric care societies, and voluntary organ and tissue donation drives. By leveraging "
        "modern web technologies, strict role-based access control, and atomic transaction architectures, the laboratory ensures that digital "
        "systems protect patient confidentiality while maximizing operational transparency for donors and clinical care teams alike."
    )
    add_p(doc, about_p3, font_size=14, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=18, line_spacing=1.5, indent=0.5)

    about_p4 = (
        "As a recognized industry internship partner for premier academic institutions affiliated with APJ Abdul Kalam Technological "
        "University, Health-Tech Innovations actively collaborates with postgraduate researchers to bridge the gap between theoretical software "
        "paradigms and robust, real-world community deployments."
    )
    add_p(doc, about_p4, font_size=14, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=0, line_spacing=1.5, indent=0.5)

    # ──────────────────────────────────────────────────────────────────────────
    # 5. ACKNOWLEDGEMENT (EXACTLY 1 PAGE, FONT 12, 1.5 LINE SPACING)
    # ──────────────────────────────────────────────────────────────────────────
    doc.add_page_break()
    add_p(doc, "ACKNOWLEDGEMENT", font_size=16, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_before=15, space_after=24)

    ack_p1 = (
        "I express my profound sense of gratitude and sincere indebtedness to our respected Principal, Dr. Sabeena M V, for providing "
        "all necessary academic facilities, computational infrastructure, and institutional encouragement that made the completion of this "
    ...
    ack_p2 = (
        "I convey my deepest and heartfelt thanks to Mrs. Sruti Sudevan, Head of the Department of Computer Applications, for her constant "
        "inspiration, academic leadership, and continuous encouragement throughout the duration of the MCA curriculum and during this project endeavor."
    )
    add_p(doc, ack_p2, font_size=12, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=14, line_spacing=1.5, indent=0.5)

    ack_p3 = (
        "I take immense privilege in expressing my sincere gratitude to my Project Guide and Coordinator, Mrs. Aiswarya N, Assistant Professor, "
        "Department of Computer Applications, and Mrs. Sruti Sudevan, for their indispensable guidance, technical mentorship, and patient supervision. "
        "Their constructive criticisms, insightful suggestions, and thorough evaluations at every phase of system modeling, design, and testing "
    ...
    ack_p4 = (
        "I also extend my sincere gratitude to all the teaching and non-teaching faculty members of the Department of Computer Applications "
    ...
    ack_p5 = (
        "I express my loving thanks to my parents and family members whose unwavering moral support, sacrifices, and continuous prayers "
        "have been the bedrock of my life and education. I also express my warm appreciation to my batchmates and friends for their collaborative "
    ...
    add_p(doc, ack_p6, font_size=12, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=40, line_spacing=1.5, indent=0.5)
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # 6. ABSTRACT (EXACTLY 1 PAGE, FONT 12, 1.5 LINE SPACING)
    # ──────────────────────────────────────────────────────────────────────────
    doc.add_page_break()
    add_p(doc, "ABSTRACT", font_size=16, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_before=15, space_after=24)

    abs_p1 = (
        "Chemotherapy-induced alopecia (hair loss) is widely recognized in oncological care as one of the most psychologically distressing "
        "side effects for cancer patients, precipitating profound loss of personal dignity, clinical anxiety, and social alienation. While thousands "
        "of citizens across society wish to donate their natural hair for medical wig-making initiatives, traditional donation approaches in Kerala "
        "and nationally remain unorganized. Existing initiatives rely on informal WhatsApp groups, sporadic in-person drop-offs, and unlinked "
        "spreadsheets. This absence of centralized logistics creates acute operational bottlenecks: generous donors have zero visibility into parcel "
        "arrivals; non-governmental organizations (NGOs) receive unsorted specimens lacking vital metadata; and immunocompromised cancer patients "
        "are forced to physically travel to offices with paper diagnostic reports to prove their condition."
    )
    add_p(doc, abs_p1, font_size=12, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=14, line_spacing=1.5, indent=0.5)

    abs_p2 = (
        "To decisively resolve these failures, this thesis presents HairFidence: Cancer Patient Hair Donation Management System, an end-to-end, "
        "secure, 3-Tier Model-View-Controller (MVC) web application. The platform digitizes, automates, and audits the entire hair donation, clinical "
        "verification, and prosthesis distribution lifecycle. Engineered using an interactive HTML5/Vanilla CSS3/ES6+ JavaScript frontend paired with a "
        "modular PHP 8.x backend engine, all transactional states are anchored in an optimized 8-table relational MySQL schema running in an Apache XAMPP "
        "server environment."
    )
    add_p(doc, abs_p2, font_size=12, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=14, line_spacing=1.5, indent=0.5)

    abs_p3 = (
        "HairFidence partitions governance across four discrete role modules: System Administrator (platform vetting and grievance resolution), "
        "Healthcare NGOs (physical parcel audits, clinical diagnostic report verification, and community drives), Donors (specification authoring "
        "and multi-stage pipeline tracking), and Patients (secure medical report uploading and catalog browsing). A critical technical contribution "
        "is the implementation of Pessimistic Concurrency Locking via SELECT ... FOR UPDATE wrapped within atomic PDO database transactions, strictly "
        "preventing double-booking race conditions during simultaneous patient requests. Rigorous unit, integration, and black-box test suites validate "
        "that the system delivers robust data security, zero-cost wig access for cancer survivors, and total transparency for civic donors."
    )
    ...
    # ──────────────────────────────────────────────────────────────────────────
    doc.add_page_break()
    add_p(doc, "CONTENTS", font_size=16, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_before=15, space_after=20)
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # SECTION 1 BREAK: BODY CHAPTERS (RUNNING HEADER & FOOTER)
    # ──────────────────────────────────────────────────────────────────────────
    ...
    p_hdr.text = "HAIRFIDENCE: Cancer Patient Hair Donation Management System\t\t"
    p_hdr.runs[0].font.name = "Times New Roman"
    p_hdr.runs[0].font.italic = True
    p_hdr.runs[0].font.size = Pt(10)
    p_hdr.runs[0].font.color.rgb = RGBColor(0x44, 0x44, 0x44)
    ...
    p_ftr.text = "Dept of Computer Applications, AWH Engineering College\t\tPage "
    ...
    t_enh = doc.add_table(rows=len(enhancements_data), cols=3)
    t_enh.alignment = WD_TABLE_ALIGNMENT.CENTER
    for r_i, r_data in enumerate(enhancements_data):
        for c_i, val in enumerate(r_data):
            cell = t_enh.cell(r_i, c_i)
    ...
    t_hw = doc.add_table(rows=len(hw_specs), cols=3)
    t_hw.alignment = WD_TABLE_ALIGNMENT.CENTER
    for r_i, r_data in enumerate(hw_specs):
        for c_i, val in enumerate(r_data):
    ...
    t_sw = doc.add_table(rows=len(sw_specs), cols=2)
    t_sw.alignment = WD_TABLE_ALIGNMENT.CENTER
    for r_i, r_data in enumerate(sw_specs):
        for c_i, val in enumerate(r_data):
    ...
    if os.path.exists(UML_PATH):
        ...
        p_uml.paragraph_format.space_after = Pt(6)
        p_uml.add_run().add_picture(UML_PATH, width=Inches(5.5))
        add_p(doc, "Figure 13.1: UML Use Case Diagram for HairFidence System", font_size=10, italic=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=16)
    ...
    t_dbs = doc.add_table(rows=len(db_tables_summary), cols=4)
    t_dbs.alignment = WD_TABLE_ALIGNMENT.CENTER
    for r_i, r_data in enumerate(db_tables_summary):
        for c_i, val in enumerate(r_data):
    ...
    add_p(doc, "• First Normal Form (1NF): All attribute domains contain exclusively atomic (indivisible) values. Attributes such as hair_length, hair_type, and image_url store single scalar values. There are zero multi-valued columns or repeating groups.\n"
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # CHAPTER 19: APPENDIX (INCLUDING UI SCREENSHOTS)
    # ──────────────────────────────────────────────────────────────────────────
    ...
    for filename, caption in screenshots_info:
        ...
        if os.path.exists(img_path):
            ...
            add_p(doc, caption, font_size=10, italic=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=14)
